...
async function handlePost(request: NextRequest, user: AuthenticatedUser) {
    try {
        ...
        const { full_name, id_card_number, gender, phone_number, date_of_birth, address, new_password } = parsed.data;
        ...
        try {
            ...
            // 2. Update participant profile record (id_card, gender, contact, clear must_change_password)
            await connection.execute(
                `UPDATE participant_profiles 
                 SET id_card_number = ?, gender = ?, phone_number = ?, date_of_birth = ?, address = ?, initial_password = ?, must_change_password = 0 
                 WHERE user_id = ?`,
                [
                    cleanIdCard,
                    gender,
        ...
        await logger.audit(
            ...
            'users',
            user.id,
            { full_name, has_id_card: true, gender },
            'SECURITY'
        );
