...
export default function ProfilePage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.full_name.trim()) {
            ...
            toast.warning('Peringatan', { description: 'Nama lengkap wajib diisi sesuai dokumen resmi.' });
            return;
        }

        if (!formData.id_card_number || formData.id_card_number.trim().length < 6) {
            setMessage({ type: 'error', text: 'Nomor NIK KTP / Paspor wajib diisi (minimal 6 karakter)' });
            toast.warning('NIK / Paspor Wajib', { description: 'Nomor identitas resmi diperlukan untuk penerbitan dokumen sertifikat.' });
            return;
        }
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-wrap items-center gap-3 self-start lg:self-auto shrink-0">
                        ...
                        {participantInfo.nip && (
                            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-center justify-between gap-3">
                                <div className="min-w-0">
                                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">NIP Peserta</p>
                                    <p className="text-xs font-mono font-bold text-slate-900 mt-0.5 tracking-wider">{participantInfo.nip}</p>
                                </div>
                                <button
                                    type="button"
                                    onClick={handleCopyNip}
                                    className="p-1.5 rounded-lg bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 transition-colors cursor-pointer shrink-0"
                                    title="Salin NIP"
                                >
                                    {copiedNip ? <Tick01Icon size={15} className="text-emerald-600" /> : <Copy01Icon size={15} />}
                                </button>
                            </div>
                        )}
                        ...
                        {formData.id_card_number && (
                            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 min-w-[130px]">
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                                    <IdIcon size={12} className="text-slate-400" />
                                    NIK / Paspor
                                </p>
                                <p className="text-xs font-mono font-bold text-slate-900 mt-0.5 tracking-wider uppercase truncate">
                                    {formData.id_card_number}
                                </p>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div className="space-y-1.5 sm:col-span-2">
                                <div className="flex items-center justify-between">
                                    <label className="text-xs font-semibold text-slate-700">
                                        NIK KTP / No. Paspor <span className="text-red-500">* (Wajib)</span>
                                    </label>
                                    <span className="text-[10px] text-slate-400 font-mono">16 digit NIK atau kode Paspor</span>
                                </div>
                                <div className="relative">
                                    <IdIcon size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                                    <input
                                        type="text"
                                        name="id_card_number"
                                        required
                                        ...
                                        placeholder="Masukkan 16 digit NIK KTP atau Nomor Paspor"
                                        className="w-full pl-10 pr-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-900 text-xs focus:border-slate-400 focus:outline-none transition-colors font-mono uppercase tracking-wider"
                                    />
                                </div>
                                <p className="text-[10px] text-slate-500">
                                    Identitas kependudukan/paspor resmi digunakan untuk verifikasi kelulusan dan penomoran ijazah/sertifikat.
                                </p>
                            </div>
